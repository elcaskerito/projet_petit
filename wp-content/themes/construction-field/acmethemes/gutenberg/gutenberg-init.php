...
function construction_field_dynamic_editor_styles(){
	...
	if( isset($construction_field_customizer_all_values['construction-field-link-color'])){
        $construction_field_link_color               = esc_attr( $construction_field_customizer_all_values['construction-field-link-color'] );
        $custom_css .= "
	        .wp-block a{
	            color: {$construction_field_link_color};
	        }";
    }
    if( isset($construction_field_customizer_all_values['construction-field-link-hover-color'])){
        $construction_field_link_hover_color         = esc_attr( $construction_field_customizer_all_values['construction-field-link-hover-color'] );
...
function construction_field_block_editor_styles() {
	wp_enqueue_style( 'construction-field-googleapis', '//fonts.googleapis.com/css?family=Poppins:100,100i,200,200i,300,300i,400,400i,500,500i,600,600i,700,700i,800,800i,900,900i', array(), null );
	wp_enqueue_style( 'construction-field-block-editor-styles', get_template_directory_uri() . '/acmethemes/gutenberg/gutenberg-edit.css', false, '1.0' );

	/**
	 * Styles from the customizer
	 */
	wp_add_inline_style( 'construction-field-block-editor-styles', construction_field_dynamic_editor_styles() );
}
...
function construction_field_gutenberg_scripts() {
	wp_enqueue_style( 'construction-field-block-front-styles', get_template_directory_uri() . '/acmethemes/gutenberg/gutenberg-front.css', false, '1.0' );
	wp_style_add_data( 'construction-field-block-front-styles', 'rtl', 'replace' );
}
